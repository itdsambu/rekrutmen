...
// --------------- KARYAWAN ---------------
    ...
    function KaryawanFilter(){
        $cari = $this->input->post('val');
        $data  = $this->m_datakaryawan->selectAllKaryawanNama($cari);
        $html = '';
        foreach($data as $row){
            $html .= '<tr> 
            <td hidden>'.$row->NIK.'</td>
            <td>' .$row->NAMA.'</td>
            <td>' .$row->TEMPATLHR.','. date('d-M-Y', strtotime($row->TGLLAHIR)).'</td>
            <td>' .$row->NamaSuku.'</td>
            <td>'.$row->AGAMA.'</td>
            <td>' .$row->ALAMATS.'</td>
            <td>' .$row->ALAMATR.'</td>
            <td>
                ...
                <a title="Print" data-rel="tooltip" href=" site_url("datakaryawan/viewKaryawan/"'.$row->NIK.')" target="_blank" class="btn btn-minier btn-round btn-warning">
