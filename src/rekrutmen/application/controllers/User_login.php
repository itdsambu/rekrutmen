...
class User_login extends CI_Controller
{
    ...
    //         'NotActive'        => $this->input->post('txtStatus'),
    //         'InActive'         => $this->input->post('txtStatus'),
    //         'telenotif'        => $this->input->post('txtTelenotif'),
    //         'telenotiftim'       => $this->input->post('txtTelenotifTim'),
    //         'notifaccess'       => $this->input->post('txtnotifaccess'),
    //         'UpdatedBy'        => strtoupper($this->session->userdata('username')),
    //         'UpdatedDate'      => date('Y-m-d H: i: s')
    //     );
    ...
    function updateUserLogin()
    {
        ...
        // Ambil dan sanitasi LoginID dari GET
        $loginID = $this->input->get('id', true);
        $loginID = trim($loginID);

        // Validasi loginID biar cuma alfanumerik + underscore/dash
        if (!$loginID || !preg_match('/^[a-zA-Z0-9_-]+$/', $loginID)) {
            redirect('user_login/listUserLogin?msg=invalid_id');
            exit;
        }

        // Ambil data POST + XSS filter CI
        $namaLengkap   = $this->input->post('txtNamaLengkap', true);
        $dept          = $this->input->post('txtDept', true);
        $groupID       = $this->input->post('txtGrupID', true);
        $screening     = $this->input->post('txtScreening', true);
        $status        = $this->input->post('txtStatus', true);
        $telenotif     = $this->input->post('txtTelenotif', true);
        $telenotifTim  = $this->input->post('txtTelenotifTim', true);
        $notifaccess   = $this->input->post('txtnotifaccess', true);

        // Trim semua
        $namaLengkap  = trim($namaLengkap);
        $dept         = strtoupper(trim($dept));
        $groupID      = trim($groupID);
        $screening    = trim($screening);
        $status       = trim($status);
        $telenotif    = trim($telenotif);
        $telenotifTim = trim($telenotifTim);
        $notifaccess  = trim($notifaccess);

        // Optional: limit panjang biar gak bisa abuse payload panjang
        $namaLengkap  = substr($namaLengkap, 0, 100);
        $dept         = substr($dept, 0, 20);
        $groupID      = substr($groupID, 0, 20);

        // Pastikan status cuma nilai tertentu (contoh 0/1)
        if (!in_array($status, ['0', '1'], true)) {
            $status = '0';
        }

        // Pastikan notif juga cuma 0/1
        if (!in_array($telenotif, ['0', '1'], true)) {
            $telenotif = '0';
        }

        if (!in_array($telenotifTim, ['0', '1'], true)) {
            $telenotifTim = '0';
        }

        if (!in_array($notifaccess, ['0', '1'], true)) {
            $notifaccess = '0';
        }

        // Screening juga biasanya boolean
        if (!in_array($screening, ['0', '1'], true)) {
            $screening = '0';
        }

        // Username session juga disanitasi
        $updatedBy = strtoupper(trim($this->session->userdata('username')));
        $updatedBy = preg_replace('/[^A-Z0-9_-]/', '', $updatedBy);

        $data = array(
            'NamaUser'         => $namaLengkap,
            'DeptID'           => $dept,
            'GroupID'          => $groupID,
            'AnggotaScreening' => $screening,
            'NotActive'        => $status,
            'InActive'         => $status,
            'telenotif'        => $telenotif,
            'telenotiftim'     => $telenotifTim,
            'notifaccess'      => $notifaccess,
            'UpdatedBy'        => $updatedBy,
            'UpdatedDate'      => date('Y-m-d H:i:s')
        );

        $userID = array('LoginID' => $loginID);

        $result = $this->m_user_login->updateUserLogin($userID, $data);

        // Catatan: logic kamu kebalik. Biasanya true = sukses.
        if ($result) {
            redirect('user_login/listUserLogin?msg=success_edit');
        } else {
            redirect('user_login/listUserLogin?msg=failed_edit');
    ...
    function savenewuser()
    {
        ...
        if ($this->input->post('txtDeptID') == '58') {
            $idDep = '24';
        } else {
            $idDep = $this->input->post('txtDeptID');
        }


        if ($cek == false) {
            $dataa = array(
                'NIK'              => trim($this->input->post('txtNIK')),
                ...
                'telenotif'        => $this->input->post('txtTelenotif'),
                'telenotiftim'       => $this->input->post('txtTelenotifTim'),
                'notifaccess'       => $this->input->post('txtaccessnotif'),
