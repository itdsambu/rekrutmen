<?php if (!defined('BASEPATH')) exit('No direct script access allowed');

/* 
 * Author : ITD15
 */

class ScreeningByTim extends CI_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model(array('darurat', 'm_dashboard'));
        $status = $this->darurat->getStatus();
        if ($status === 1 && $this->session->userdata('userid') !== 'ismo_adm') {
            redirect(site_url('maintenanceControl'));
        }

        date_default_timezone_set("Asia/Jakarta");
        if (!$this->session->userdata('userid')) {
            redirect('login');
        }

        $screenTeam = $this->m_dashboard->getStatusScreenTeam();
        $dept   = $this->session->userdata('dept');
        $cekDept    = $this->m_dashboard->getDeptScreenTeam($dept);
        // if($screenTeam == 0 || $cekDept == FALSE && $this->session->userdata('userid') != 'taufik' && $this->session->userdata('userid') != 'pramujito'){
        //     redirect('welcome/notAksesScreen');
        // }

        $this->load->model('m_screening');
    }

    public function index()
    {
        // print_r($this->session->userdata());
        // die;
        $nowOL = $this->session->userdata('username');
        $dept   = $this->session->userdata('dept');
        $data['_getTK'] = $this->m_screening->listTenagaKerja($nowOL, $dept);
        // if ($this->session->userdata('userid') == 'KIKI') {
        //     $this->template->display('registrasi/screening/tim_', $data);
        //     return;
        // }
        $this->template->display('registrasi/screening/tim', $data);
    }

    public function DivUtility()
    {
        $nowOL          = $this->session->userdata('username');
        $dept           = $this->session->userdata('dept');
        $data['_getTK'] = $this->m_screening->listTenagaKerja($nowOL, $dept, 1);
        $data['isDiv']  = 1;
        $this->template->display('registrasi/screening/tim', $data);
    }

    public function DivP2K3()
    {
        $nowOL = $this->session->userdata('username');
        $dept   = $this->session->userdata('dept');
        $data['_getTK'] = $this->m_screening->listTenagaKerja($nowOL, $dept, 2);
        $data['isDiv'] = 2;
        $this->template->display('registrasi/screening/tim', $data);
    }
    public function DivELC()
    {
        $nowOL = $this->session->userdata('username');
        $dept   = $this->session->userdata('dept');
        $data['_getTK'] = $this->m_screening->listTenagaKerja($nowOL, $dept, 3);
        $data['isDiv'] = 3;
        $this->template->display('registrasi/screening/tim', $data);
    }

    public function DivHED()
    {
        $nowOL = $this->session->userdata('username');
        $dept   = $this->session->userdata('dept');
        $data['_getTK'] = $this->m_screening->listTenagaKerja($nowOL, $dept, 4);
        $data['isDiv'] = 4;
        $this->template->display('registrasi/screening/tim', $data);
    }

    function screenTim()
    {
        if ('IS_AJAX') {
            $kode   = $this->input->post('kode');
            $div    = $this->input->post('div');
            $data['datatk']     = $this->m_screening->getDetailTK($kode)->result();
            $data['hasilLulus'] = $this->m_screening->getHasilLulus($kode);
            $data['hasilTLulus'] = $this->m_screening->getHasilTIdakLulus($kode);
            $data['isDiv'] = $div;
            $this->load->view('registrasi/screening/screenTim', $data);
        }
    }

    function simpan()
    {
        $data = array(
            'HeaderID'      => $this->input->post('txtHeaderID'),
            'Dept'          => $this->input->post('txtDept'),
            'ScreeningBy'   => $this->input->post('txtName'),
            'ScreeningDate' => date('Y-m-d H:m:i'),
            'ScreeningKet'  => $this->input->post('txtKeterangan'),
            'Kenal'         => $this->input->post('txtKenal'),
            'PernahKerja'   => $this->input->post('txtKerja'),
            'Lulus'         => $this->input->post('txtRekomen'),
            'Jeda'          => $this->input->post('txtJeda')
        );

        $this->m_screening->simpanScreeningTim($data);

        $hrdID  = $this->input->post('txtHeaderID');
        $lulus  = $this->m_screening->getHasilLulus($hrdID);
        $gagal  = $this->m_screening->getHasilTIdakLulus($hrdID);

        if ($lulus >= $gagal) {
            $info   = array(
                'ScreeningHasil' => 1
            );
            $this->m_screening->updateLulus($hrdID, $info);
        } else {
            $info   = array(
                'ScreeningHasil' => 0
            );
            $this->m_screening->updateLulus($hrdID, $info);
        }
        $hasilScreen = $this->m_screening->getHasilScreen($hrdID);

        if ($hasilScreen == 'complite') {
            $info   = array(
                'ScreeningComplete' => 1
            );
            $this->m_screening->updateLulus($hrdID, $info);
        }

        redirect('screeningByTim/index?msg=Success');
    }

    function save()
    {
        // Ambil data dari POST
        $data = array(
            'HeaderID'      => $this->input->post('txtHeaderID'),
            'Dept'          => $this->input->post('txtDept'),
            'ScreeningBy'   => $this->input->post('txtName'),
            'ScreeningDate' => date('Y-m-d H:i:s'), // perbaikan format jam: H:i:s (bukan H:m:i)
            'ScreeningKet'  => $this->input->post('txtKeterangan'),
            'Kenal'         => $this->input->post('txtKenal'),
            'PernahKerja'   => $this->input->post('txtKerja'),
            'Lulus'         => $this->input->post('txtRekomen'),
            'Jeda'          => $this->input->post('txtJeda')
        );
        // echo json_encode(['status' => 'success']);
        // return;

        // Simpan data screening
        $simpan = $this->m_screening->simpanScreeningTim($data);

        // Cek apakah insert berhasil
        if ($simpan) {

            $hrdID  = $this->input->post('txtHeaderID');
            $lulus  = $this->m_screening->getHasilLulus($hrdID);
            $gagal  = $this->m_screening->getHasilTIdakLulus($hrdID);

            if ($lulus >= $gagal) {
                $info = array('ScreeningHasil' => 1);
            } else {
                $info = array('ScreeningHasil' => 0);
            }

            $update = $this->m_screening->updateLulus($hrdID, $info);

            if ($update) {
                $hasilScreen = $this->m_screening->getHasilScreen($hrdID);

                if ($hasilScreen == 'complite') {
                    $updateComplete = $this->m_screening->updateLulus($hrdID, array('ScreeningComplete' => 1));
                    if (!$updateComplete) {
                        echo json_encode(array('status' => 'failed', 'message' => 'Gagal update ScreeningComplete'));
                        exit;
                    }
                }

                // Semua berhasil
                echo json_encode(array('status' => 'success'));
            } else {
                echo json_encode(array('status' => 'failed', 'message' => 'Gagal update ScreeningHasil'));
            }
        } else {
            echo json_encode(array('status' => 'failed', 'message' => 'Gagal insert Screening'));
        }
    }

    function AppDiv()
    {
        $hdrID  = $this->input->get('headerid');
        $info = array(
            'AppDivStatus'  => 1,
            'AppDivBy'      => $this->session->userdata('username'),
            'AppDivDate'    => date('Y-m-d H:m:i'),
        );
        $this->m_screening->updateLulus($hdrID, $info);
        #echo $hdrID;
        redirect('screeningByTim/DivUtility?msg=Successs');
    }

    function AppP2K3()
    {
        $hdrID = $this->input->post('P2K3HeaderID');
        $AppP2K3Catatan = $this->input->post('AppP2K3Catatan');
        // var_dump($hdrID,$AppP2K3Catatan); die;
        // if (isset($_POST['btnproses']) == 'btn_disapp') {
        //     # code...
        //     $info = array(
        //         'AppP2K3Status'  => 0,
        //         'AppP2K3By'      => $this->session->userdata('userid'),
        //         'AppP2K3Date'    => date('Y-m-d H:m:i'),
        //         'AppP2K3Catatan' => $AppP2K3Catatan,
        //     );
        // }
        // else {
        # code...
        $info = array(
            'AppP2K3Status'  => 1,
            'AppP2K3By'      => $this->session->userdata('userid'),
            'AppP2K3Date'    => date('Y-m-d H:m:i'),
            'AppP2K3Catatan' => $AppP2K3Catatan,
        );
        // }
        $this->m_screening->updateLulus($hdrID, $info);
        redirect('screeningByTim/DivP2K3?msg=Successss');
    }

    function AppELC()
    {
        $hdrID = $this->input->post('ELCHeaderID');
        $AppELCCatatan = $this->input->post('AppELCCatatan');
        // var_dump($hdrID,$AppELCCatatan); die;
        $info = array(
            'AppELCStatus'  => 1,
            'AppELCBy'      => $this->session->userdata('userid'),
            'AppELCDate'    => date('Y-m-d H:m:i'),
            'AppELCCatatan' => $AppELCCatatan,
        );
        $this->m_screening->updateLulus($hdrID, $info);
        redirect('screeningByTim/DivELC?msg=SuccessssS');
    }

    function AppHED()
    {
        $hdrID = $this->input->post('HEDHeaderID');
        $AppHEDCatatan = $this->input->post('AppHEDCatatan');
        // var_dump($hdrID,$AppHEDCatatan); die;
        $info = array(
            'AppHEDStatus'  => 1,
            'AppHEDBy'      => $this->session->userdata('userid'),
            'AppHEDDate'    => date('Y-m-d H:m:i'),
            'AppHEDCatatan' => $AppHEDCatatan,
        );
        $this->m_screening->updateLulus($hdrID, $info);
        redirect('screeningByTim/DivHED?msg=SuccessssSS');
    }


    // ========= List Tenaga KErja
    function detailtk()
    {
        if ('IS_AJAX') {
            $kode = $this->input->post('kode');
            $data['datatk'] = $this->m_screening->getDetailTK($kode)->result();
            $this->load->view('registrasi/screening/detail', $data);
        }
    }
}

/* End of file screeningByTim.php */
/* Location: ./application/controllers/screeningByTim.php */