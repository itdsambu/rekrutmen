...
class BonPiutang extends CI_Controller {
	...
	public function storebon(){
		...
		$query = $this->m_bonpiutang->getminmax($firstitem->idperusahaan);
		$row = $query->row();
		$ismin = $row->ismin;
		$ismax = $row->ismax;		
	    ...
	    foreach( $data as $itemmodel ){
			...
			if($isexists){
				...
				$param = array('fixno'=>$itemmodel->fixno,'bon'=>$bontk,'periode'=>$itemmodel->periode,
	...
	public function havebontk(){
		...
		$query = $this->db->query("select *,addonedaylock=convert(varchar(10),DATEADD(DAY,1,lockperiode),126) from psgborongan..vwcurperiode");
		$row = $query->row();
