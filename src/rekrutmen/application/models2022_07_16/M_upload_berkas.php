...
//        return $query->result();
        if ($idpemborong > 0){
            if ($filter_status == 'lengkap') {
                $query = $this->db->query("SELECT * FROM vwListBerkas WHERE GeneralStatus = 0 AND KTP IS NOT NULL AND CV IS NOT NULL AND Lamaran IS NOT NULL AND Ijazah IS NOT NULL AND Transkrip IS NOT NULL AND " . "CVNama IN (SELECT Perusahaan FROM vwMstPemborong WHERE IDPerusahaan = '".$idpemborong."' ) ");
            } else if ($filter_status == 'minimal') {
                $query = $this->db->query("SELECT * FROM vwListBerkas WHERE GeneralStatus = 0 AND KTP IS NOT NULL AND (CV IS NULL OR Lamaran IS NULL OR Ijazah IS NULL OR Transkrip IS NULL) AND " . "CVNama IN (SELECT Perusahaan FROM vwMstPemborong WHERE IDPerusahaan = '".$idpemborong."' ) ");
            } else {
                $query = $this->db->query("SELECT * FROM vwListBerkas WHERE GeneralStatus = 0 AND KTP IS NULL AND " . "CVNama IN (SELECT Perusahaan FROM vwMstPemborong WHERE IDPerusahaan = '".$idpemborong."' ) ");
            }
        }else{
