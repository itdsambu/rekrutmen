...
//        $data['_getPemborong']= $this->m_register->get_pemborong_bygroup($idpemborong)->result();
    ...
    function simpanReg(){
        ...
        $jmlsaudara = $this->input->post('txtJumlahSaudara');
        if ($jmlsaudara > 0) {
            $jmlsdr = count($this->input->post('txtNamaSaudara'));
        }else{
            $jmlsdr = 0;
        }
        ...
        $sdrnama         = $this->input->post('txtNamaSaudara');
        $sdrjeniskelamin = $this->input->post('txtJekelSaudara');
        $sdrumur         = $this->input->post('txtUmurSaudara');
        $sdrpekerjaan    = $this->input->post('txtPekerjaanSaudara');
        $sdrperusahaan   = $this->input->post('txtPrusahaanSaudara');
        $sdrjabatan      = $this->input->post('txtJabatanSaudara');

        $pendidikantingkat      = $this->input->post('txtTingkatanPendidikan');
        $pendidikannama         = $this->input->post('txtNmSekolahPendidikan');
        $pendidikanjurusan      = $this->input->post('txtJurusanPendidikan');
        $pendidikantahun1       = $this->input->post('txtThnMasukPendidikan');
        $pendidikantahun2       = $this->input->post('txtThnLulusPendidikan');
        $pendidikanlulus        = $this->input->post('txtLulusPendidikan');
        ...
        if ($confirm == 0){
            ...
            elseif ($cekTKPem == true) {
                ...
                for ($i=0; $i < count($this->input->post('txtTingkatanPendidikan')); $i++) { 
                    $infopendidikan = array(
                              'HeaderID'    => 0,
                              'HeaderIDTemp'=> $hdridtemp,
                              'Tingkat'     => strtoupper($pendidikantingkat[$i]),
                              'Nama'        => strtoupper($pendidikannama[$i]),
                              'Jurusan'     => strtoupper($pendidikanjurusan[$i]),
                              'TahunMasuk'  => strtoupper($pendidikantahun1[$i]),
                              'TahunKeluar' => strtoupper($pendidikantahun2[$i]),
                              'Lulus'     => strtoupper($pendidikanlulus[$i]),
                              'CreatedBy'   => strtoupper($this->session->userdata('userid')),
                              'CreatedDate' => date('Y-m-d H:i:s')
                    );

                    echo json_encode($infopendidikan);
                    if(!$pendidikantingkat[$i]==''){
                        $this->simpan_datapddkn(0, $hdridtemp, $pendidikantingkat[$i], $infopendidikan);
                    }
                }
                ...
                for ($i=0; $i < $jmlsdr; $i++) { 
                    $infosaudara = array(
                                'HeaderID'      => 0,
                                'HeaderIDTemp'  => $hdridtemp,
                                'Nama'          => strtoupper($sdrnama[$i]),
                                'JenisKelamin'  => strtoupper($sdrjeniskelamin[$i]),
                                'Umur'          => $sdrumur[$i],
                                'Perusahaan'    => strtoupper($sdrperusahaan[$i]),
                                'Pekerjaan'     => strtoupper($sdrpekerjaan[$i]),
                                'Jabatan'       => strtoupper($sdrjabatan[$i]),
                                'CreatedBy'     => strtoupper($this->session->userdata('userid')),
                                'CreatedDate'   => date('Y-m-d H:i:s')
                    );
                    if(!$sdrnama[$i]==''){
                        $this->simpan_datasdr(0, $hdridtemp, $sdrnama[$i], $infosaudara);
                    }
                }
                ...
                for ($i=0; $i < count($this->input->post('txtTingkatanPendidikan')); $i++) { 
                    ...
                    echo json_encode($infopendidikan);
                    if(!$pendidikantingkat[$i]==''){
                        $this->simpan_datapddkn($hdrID, 0, $pendidikantingkat[$i], $infopendidikan);
                    }
                }
                ...
                // === Cek Data sAUDARA, Jika Ada Simpan
                for ($i=0; $i < $jmlsdr; $i++) { 
                    $infosaudara = array(
                                'HeaderID'      => $hdrID,
                                'HeaderIDTemp'  => 0,
                    ...
                    if(!$sdrnama[$i]==''){
                        $this->simpan_datasdr($hdrID, 0, $sdrnama[$i], $infosaudara);
                    }
                }
    ...
    function simpan_datasdr($hdrID, $hdridtemp, $sdrnama, $infosaudara){
        $detailid = $this->m_register->cek_datasdr($hdrID,$hdridtemp,$sdrnama);

        if ($detailid == 0){
            $this->m_register->simpan_datasdr($infosaudara);
        }else{
            $this->m_register->update_datasdr($detailid, $infosaudara);
        }
    }

     function simpan_datapddkn($hdrID, $hdridtemp, $pendidikantingkat, $infopendidikan){
        $detailid = $this->m_register->cek_datapendidikan($hdrID,$hdridtemp,$pendidikantingkat);

        if ($detailid == 0){
            $this->m_register->simpan_datapendidikan($infopendidikan);
        }else{
            $this->m_register->update_datapendidikan($detailid, $infopendidikan);
        }
    }
