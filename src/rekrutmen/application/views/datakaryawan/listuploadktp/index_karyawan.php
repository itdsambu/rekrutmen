<?php if (($this->session->userdata('dept') == 'PSN') || ($this->session->userdata('dept') == 'ITD')) : ?>
        ...
        <div class="col-xs-12">
            <div class="widget-box">
                <div class="widget-header">
                    <h4 class="widget-title">List Karyawan Aktif</h4>
                ...
                <div class="widget-body">
                    <div class="widget-main">
                        <div class="row">
                            ...
                            <form class="form-horizontal" role="form" method="POST" action="<?php echo base_url('datakaryawan/filterkaryawandata'); ?>">
                                        ...
                                        <div class="col-sm-8">
                                            <select name="txtStatusKtp" id="inputStatusKtp" class="form-control input-sm">
                                                <option value="0" <?php if ($_selected == 0) {
                                                                        echo 'selected';
                                                                    } ?>>All Data Karyawan</option>
                                                <option value="1" <?php if ($_selected == 1) {
                                                                        echo 'selected';
                                                                    } ?>>Data Karyawan Registered</option>
                                                <option value="2" <?php if ($_selected == 2) {
                                                                        echo 'selected';
                                                                    } ?>>Data Karyawan Unregistered</option>
                                                option
                                            </select>
                                        </div>
                                        <script>
                                            $('#inputStatusKtp').change(function() {
                                                var val = this.value;
                                                window.location = '<?php echo site_url(); ?>datakaryawan/listuploadktpkaryawan/' + val;
                                            });
                                        </script>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-4 control-label no-padding-right">No. Regno</label>
                                        <div class="col-sm-8">
                                            <input name="txtRegno" id="inputRegno" type="number" class="form-control input-sm" autocomplete="off" />
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-8">
                                            <select name="txtDept" id="inputDept" class="form-control input-sm">
                                                ...
                                                <?php foreach ($all_dept as $dept) { ?>
                                                    <option value="<?php echo $dept->deptabbr; ?>" <?php if ($_selectedDept == $dept->deptabbr) {
                                                                                                        echo 'selected';
                                                                                                    } ?>><?php echo $dept->deptabbr; ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div id="data" class="col-xs-12 table-responsive">
                                <table <?php if (!empty($_selectWhere)) {
                                    ...
                                    <thead>
                                        <tr>
                                            <th class="excludeExport">RegNo</th>
                                            <th>Departemen</th>
                                            <th>NAMA</th>
                                            <th>TEMPAT/TANGGAL LAHIR</th>
                                            <th class="excludeExport">NO KTP RO</th>
                                            <th class="excludeExport">NO KTP PAYROLL</th>
                                            <th>FILE KTP</th>
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($_selectWhere as $row) :
                                            ...
                                            if ($_selected == 0) {
                                                ...
                                                } else if ($row->HeaderID == null && file_exists(FCPATH . 'dataupload/berkas/ktp_fixno/1_' . $row->regno . '_ktp.pdf')) {
                                                    $checkktp = 'File KTP Tersedia';
                                                } else {
                                                ...
                                                if (file_exists(FCPATH . 'dataupload/berkas/ktp_fixno/1_' . $row->regno . '_ktp.pdf')) {
                                                    $checkktp = 'File KTP Tersedia';
                                                } else {
                                                    $checkktp = 'File KTP Tidak Tersedia';
                                                    $style_notif2 = 'style="background: red"';
                                            ...
                                            $style_notif = "";
                                            if (TRIM($row->ro_ktp) != TRIM($row->payroll_ktp)) {
                                                $style_notif = 'style="background: yellow"';
                                            }
                                        ?>
                                            <tr class="info" data-id="<?php echo $row->HeaderID ?>">
                                                <td class="regno excludeExport"><?php echo $row->regno ?></td>
                                                <td><?php echo $row->deptabbr ?></td>
                                                <td class="nama"><?php echo $row->nama ?></td>
                                                <td><?php echo $row->tempatlhr . ", " . date('d-M-Y', strtotime($row->tgllahir)) ?></td>
                                                <td <?= $style_notif ?> class="excludeExport"><?php echo $row->ro_ktp ?></td>
                                                <td <?= $style_notif ?> class="excludeExport"><?php echo $row->payroll_ktp ?></td>
                                                ...
                                                <td class="text-left excludeExport">
                                                    <div class="btn-group">
                                                        ...
                                                        <a title="Upload KTP" data-rel="tooltip" href="<?php echo site_url('datakaryawan/doUploadKaryawanKTP') . "?id=" . $row->HeaderID . "&nama=" . $row->nama . "&regno=" . $row->regno; ?>" class="upload_ktp btn btn-minier btn-round btn-success">
    ...
    <script src="<?php echo base_url(); ?>rekrutmen/assets/table2excel.js"></script>
    <script src="<?php echo base_url(); ?>rekrutmen/assets/js/dataTables/jquery.dataTables.js"></script>
    <script src="<?php echo base_url(); ?>rekrutmen/assets/js/dataTables/jquery.dataTables.bootstrap.js"></script>
    <script src="<?php echo base_url(); ?>rekrutmen/assets/js/dataTables/extensions/TableTools/js/dataTables.tableTools.js"></script>
    <script src="<?php echo base_url(); ?>rekrutmen/assets/js/dataTables/extensions/ColVis/js/dataTables.colVis.js"></script>
    <script type="text/javascript" src="<?php echo base_url(); ?>rekrutmen/assets/jqv/jquery.tablesorter.min.js"></script>
    <script>
        ...
        $(document).ready(function() {
            $("#btnExport").click(function() {
                ...
                tableToExcel("myTable", "Sheet 1", "KTP Karyawan");
            ...
            $("#myTable").on("click", ".detail_ktp", function() {
                ...
                var regnoid = $(this).closest('tr').find(".regno").text().trim();
                console.log(id, name, tk, regnoid);

                document.getElementById('titleModal').textContent = "File " + name + " dari saudara, " + tk + "";
                $.ajax({
                    url: "<?php echo site_url('datakaryawan/viewDocsKaryawan'); ?>",
                    type: "POST",
                    data: "kode=" + id + "&nama=" + name + "&regno=" + regnoid,
                    datatype: "json",
                    cache: false,
