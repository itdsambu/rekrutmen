...
<div class="row">
    <div class="col-xs-12">
        ...
        echo form_open('issue/saveIssue_/borongan', $att); ?>
        <fieldset>
...
<script>
    ...
    $('#txtSubJabatan').change(function(e) {
        ...
        // console.log('sub :', txtSubJabatan);
        // console.log('target :', inputTargetBongkar);

        if (inputDept && txtSubJabatan) {

            $.ajax({
                url: '<?= base_url() ?>issue/get_idealbor_new',
                dataType: 'json',
                data: {
                    id: inputDept,
                    subJabatanID: txtSubJabatan,
                    idTargetBongkar: inputTargetBongkar,
                },
                type: 'POST',
    ...
    $('#formborongan').validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                url: '<?= base_url() ?>issue/cekfirstbor_',
                dataType: 'json',
                cache: false,
                contentType: false,
                processData: false,
    ...
    */
    ...
    function getDept() {
        $.ajax({
            url: '<?= base_url() ?>issue/getDept',
            dataType: 'json',
            type: 'POST',
            success: function(res) {
                console.log(res);
                if (res.status == 200) {
                    $('#inputDept').html(res.data)
                } else {
