...
$(function(){
	$('#selectProvinsi').on('change',function(){
			let idp = $(this).find(":selected").val();
			$.ajax({
			    type:'GET',
				dataType:'json',
		...
		$('#selectKabupaten').on('change',function(){
			let idp = $('#selectProvinsi').find(":selected").val();
			let ikab = $(this).find(":selected").val();
			$.ajax({
			    type:'GET',
				dataType:'json',
				...
				success	: function(d,r,x){
					data = d.data;
					$('#selectKecamatan').empty().html('<option value="">Silahkan pilih Kabupaten</option>');
					for(i=0;i<data.length;i++){
