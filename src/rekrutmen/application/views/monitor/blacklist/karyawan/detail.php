<h5 class="row header smaller lighter red">
	<span class="col-sm-8">
		...
		<strong> Informasi Karyawan Bermasalah</strong>
	</span><!-- /.col -->
</h5>
<?php
foreach ($data as $row) :
	...
	if ($row->CVNAMA == 'PT. PULAU SAMBU GUNTUNG') {
		if (file_exists(base_url() . 'fotos/Karyawan_regno new/' . $row->RegNo . '.png')) {
			$url = 'fotos/Karyawan_regno new/' . $row->RegNo . '.png';
		} else {
			$url = 'fotos/Karyawan_regno/' . $row->RegNo . '.jpg';
		}
	} else {

		$url =  'fotos/BORO_FIXNO/' . $row->RegNo . '.jpg';
	}
endforeach;
// $namafoto = 'dataupload/Blacklist/foto_kar/Karyawan/' . trim($nik) . '.jpg';
$namafoto = $url;


// $namafoto = '/dataupload/foto/' . $hdrid . '.jpg';
// $namafoto = FCPATH . $namafoto; // full local path
?>
<div class="row">
