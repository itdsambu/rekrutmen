...
class M_Bonpiutang extends CI_Model {
	...
	protected $tblbonawal = 'psgborongan.dbo.tblTrnBonAwal';
	...
	{
		$query = $this->exec_sql('PSGBorongan.dbo.BonTKUnLockPT',$param);
		return $query;
	}
		...
		$query = $this->datatablesql->sql_select($param,
		   ...
		   function($param){
			   $str = 'dbo.tblUtlGroupUser a left join dbo.tblUserGroupPerusahaan b on a.groupid=b.groupid '.
                      'left join PSGBorongan.dbo.tblmstperusahaan c on c.idperusahaan=b.idperusahaan';
			   return $str;
		   },
		   function($param){
	...
	public function get_periodebon(){
		$query = $this->exec_sql('PSGBorongan.dbo.getperiode',array());
		return $query;
	}
	
	//get islock
	public function islock($param){
		$query = $this->exec_sql('PSGBorongan.dbo.isLock',$param);
		$row = $query->row();	   
		return $row->isLock;
	}

	public function getminmax($idperusahaan){
		$sql = "select ismin=ISNULL(j.max3month,200000),ismax=isnull(j.maxgeneral,90000000) from PSGBorongan.dbo.tblMstPerusahaan p " .
		       " left join PSGBorongan.dbo.tblMstMaxInputBon j on j.idborongan =p.IDPerusahaan " .
	...
	public function gettkminmaxbon($param){
		$query = $this->datatablesql->sql_select($param,
			function($param){
				$str = "PSGBorongan.dbo.tblMstPerusahaan p " .
					   "left join PSGBorongan.dbo.tblMstMaxInputBon j on j.idborongan =p.IDPerusahaan";
				return $str;
			},
			function($param){
			...
			,
			function($param){
				$str="IDPerusahaan in (select IDPerusahaan from PSGBorongan.dbo.tblMstPemborong)";
				return $str;
			},
			function($row){
		...
		$query = $this->datatablesql->sql_select($param,
			      ...
			      function($param){
					 ...
					 /* $str = 'PSGBorongan.dbo.tblMstTenagaKerja k ' .
							 ...
							 'join PSGBorongan.dbo.tblMstPekerjaan pek on pek.IDPekerjaan = k.IDPekerjaan ' .
	...
	public function get_tkbonprocess($param){		
		...
		$periode = $this->db->query('select addonedaylock=convert(varchar(10),DATEADD(DAY,1,lockperiode),126) from PSGBorongan.dbo.vwcurperiode');
		$rowperiode = $periode->row();
	...
	public function update_bon($param){
	    ...
	    $query = $this->exec_sql('PSGBorongan.dbo.BonTkUpdate',$params);
		return $query;
	}
	//reset bon tk
	public function reset_bontk($param){
		...
		$query = $this->exec_sql('PSGBorongan.dbo.BonTkUnlock',$param);
		return $query;
	}
	
	public function bontk_setting_getdata(){
		$query = $this->db->query('select top 1 h=bontklastday,bontkhour=CONVERT(varchar(5),bontkhour) from PSGBorongan.dbo.tblbontk_setting order by id desc');
		return $query;
	}
	
	public function bontk_setting_getdata_save($param){
		$query = $this->exec_sql('PSGBorongan.dbo.BonTkSetting',$param) ;
		$i = 1;
		return $i;
	}
	...
	public function getbontkcountmonitor($paramdb,$param){
		$query = $this->exec_sql('PSGBorongan.dbo.BonTKUCountMonitor',$param);
