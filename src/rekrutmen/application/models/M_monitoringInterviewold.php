...
class M_monitoringInterview extends CI_Model{
    ...
    function get(){
    	$query = $this->db->query("SELECT * FROM tblTrnCalonKandidatNew as A left join TblMntInterview as B ON A.CalonPelamarID = B.CalonPelamarID left join vw_mstKecamatan as C ON B.KecamatanID = C.KecamatanID where A.Interview = '1'");
    	return $query->result();
    }

    function _getListDataById($idInterview){
        $query = $this->db->query("SELECT * FROM vwCalonKandidat where IDInterview = '$idInterview'");
        return $query->result();
    }
    ...
    function getPelamar($userID){
        $query = $this->db->query("SELECT * FROM tblTrnCalonKandidatNew where CalonPelamarID = '$userID'");
        return $query->result(); 
    }
    ...
    function _getDataListBerdasarkanJurusan($jurusan){
         $query = $this->db->query("SELECT * FROM vwCalonKandidat as A left join vw_mstKecamatan as B ON A.KecamatanID = B.KecamatanID 
