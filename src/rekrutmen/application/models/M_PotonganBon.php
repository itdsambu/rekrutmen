...
class M_PotonganBon extends CI_Model
{
    ...
    function GetTK($nik, $nama, $nofix, $pbr)
    {
        $query = $this->db->query(
            "WITH tk AS (
                ...
                sembako AS ( SELECT FixNo,SUM(sisa_TKBaru) as SisaSembako FROM vwSisaPeriodeSebelumnyaSembako where FixNo = '$nofix' GROUP BY FixNo )
                ...
                b.SisaSembako AS SisaPeriodeSebelumnya 
            FROM
                tk AS a
        ...
        $query = $this->PSGBOR->query("WITH Tenaga_kerja AS (
                                                        ...
                                                        sisa_tkBaru AS (
                                                            SELECT
                                                                B.IDPemborong,
                                                                a.FixNo,
                                                                SUM ( a.SisaPeriodeLalu ) AS Sisa 
                                                            FROM
                                                                vwTrnPotonganBONPemborongSummary a Join tblMstTenagaKerja B on a.FixNo = B.fixno
                                                            WHERE
                                                                b.IDPemborong = '$pbr' 
                                                        ...
                                                        Sisa_Periode_sebelumnya_tkLama AS (
                                                            ...
                                                            B.SisaCicilan,
                                                            case 
                                                            WHEN ISNULL( B.SisaCicilan, 0 ) > 20000 THEN
                                                            ABS( ISNULL( A.SisaPotBon, 0 ) - ISNULL( B.SisaCicilan, 0 )  )
                                                            ELSE
                                                            ISNULL( A.SisaPotBon, 0 )
                                                            END as SisaPeriodeSebelumnya_tklama
                                                        FROM
    ...
    function Hapus_CicilanDtl($id)
    {
        ...
        $this->PSGBOR->where('SisaDurasi is null');
        $this->PSGBOR->where('DetailID', $id);
        $this->PSGBOR->delete('psgborongan.dbo.tblTrnPotonganCicilandtl');
    }

    function CicilanTerkakulasi($id)
    {
        $query = $this->PSGBOR->query("Select * from tblTrnPotonganCicilanhdr a
                                            left JOIN
                                            tblTrnPotonganCicilandtl b
                                            on a.HeaderID = b.HeaderID
                                            where 
                                            a.HeaderID ='$id'
                                            and
                                            b.SisaDurasi is null");
        return $query->result();
    }
        ...
        $query = $this->db->query("WITH Tenaga_kerja AS ( SELECT * FROM [192.168.3.32].psgborongan.dbo.vwMstTenagaKerja WHERE TanggalKeluar IS NULL AND FixNo = '$nofix' ),
                                        Pot_sembako AS (
                                            ...
                                            SELECT
                                                ...
                                                SUM ( a.SisaPeriodeLalu ) AS Sisa 
                                            FROM
                                                [192.168.3.32].psgborongan.dbo.vwTrnPotonganBONPemborongSummary a 
                                            Join  [192.168.3.32].psgborongan.dbo.tblMstTenagaKerja B on a.FixNo = B.fixno
                                            WHERE
                                                a.FixNo = '$nofix' 
                                                AND a.PeriodeGajian = '$periode' 
                                            GROUP BY
                                            ...
                                            SELECT DISTINCT 
                                                ...
                                                k.Sisa AS Sisa_PeriodeSebelumnya_tkbaru 
                                            FROM
                                                Tenaga_kerja AS a
                                                ...
                                                LEFT JOIN sisa_tkBaru AS k ON A.FixNo = k.FixNo
                                            ORDER BY
                                                a.Nama ASC
                                        ");
        return $query->row();
...
");
    ...
    function KhususCicilan2($nofix, $tglMulai, $periode)
    {
        $query = $this->db->query("SELECT
                                    a.*,
                                    b.NamaCicilan,
...
");
    ...
    {
        $query = $this->db->query("SELECT * FROM [192.168.3.32].psgborongan.dbo.tblTrnPotonganCicilanDtl as A left join [192.168.3.32].psgborongan.dbo.tblTrnPotonganCicilanHdr as B ON A.HeaderID = B.HeaderID where A.Nofix = '" . $nofix . "' and A.CicilanID = '" . $item_id . "' and A.SatuanID = '" . $satuan . "' and A.KategoriCicilanID = '" . $kategori . "' 	AND A.SisaDurasi != 0");
        return $query->result();
    }
