	...
	private function get_tkbonprocess_notlocked($param){
		$query = $this->datatablesql->sql_select($param,
				  ...
				  function($param){
					  ...
					  if($param['islock']==0)
					  $str = "(k.TanggalKeluar is null and (k.TanggalKeluarTemporary is null or k.TanggalKeluarTemporary is not null) and p.IDPemborong='" . $param['extradata']['pemborong'] . "')";
					  else
					  $str = '(k.TanggalKeluar is null and (k.TanggalKeluarTemporary is null or k.TanggalKeluarTemporary is not null) and pe.IDPerusahaan=0)';
				      if($stat==0)
	...
	private function get_tkbonprocess_locked($param){
		$query = $this->datatablesql->sql_select($param,
				  ...
				  function($param){
					  ...
					  $stat = $param['extradata']['bontk'];
					  if($param['islock']==1)
					     $str = "(k.TanggalKeluar is Null and (k.TanggalKeluarTemporary is null or k.TanggalKeluarTemporary is not null) and pe.IDPerusahaan='" . $param['extradata']['pemborong'] . "')";
					  else
					    $str = '(k.TanggalKeluar is Null and (k.TanggalKeluarTemporary is null or k.TanggalKeluarTemporary is not null) and pe.IDPerusahaan=0)'; 
