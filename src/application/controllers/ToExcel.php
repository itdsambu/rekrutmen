...
class ToExcel extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('m_monitor', 'm_upload_berkas', 'm_screening', 'm_blacklist', 'M_Interview', 'M_transaksi', 'M_configpermintaan'));
    }
    ...
    public function exportExcel()
    {
        ...
        $data   = $this->M_configpermintaan->toExcel($target, $dept);
        $title  = 'Management Permintaan Karyawan dan TK';
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C3', 'Departemen ID (Karyawan)')
            ->setCellValue('D3', 'Departemen ID (Borongan)')
            ->setCellValue('E3', 'Ideal Karyawan/Borongan')
            ->setCellValue('F3', 'ID Jumlah bongkar Kelapa')
            ->setCellValue('G3', 'Jumlah bongkar Kelapa')
            ->setCellValue('H3', 'ID Sub Jabatan Karyawan')
            ->setCellValue('I3', 'ID Sub Jabatan Borongan')
            ->setCellValue('J3', 'Sub Jabatan')
            ->setCellValue('K3', 'Real Karyawan')
            ->setCellValue('L3', 'Real Borongan')
            ->setCellValue('M3', 'Total Karyawan/Borongan')
            ->setCellValue('N3', 'Created By')
            ->setCellValue('O3', 'Periode');
        ...
        foreach ($data as $row) :
            ...
            $ex->setCellValue('C' . $counter, $row->DeptID);
            $ex->setCellValue('D' . $counter, $row->DeptIDBor);
            $ex->setCellValue('E' . $counter, $row->IKry);
            $ex->setCellValue('F' . $counter, $row->id_master_bongkar_kelapa);
            $ex->setCellValue('G' . $counter, $row->jumlah);
            $ex->setCellValue('H' . $counter, $row->IDSubJabatan_Kar);
            $ex->setCellValue('I' . $counter, $row->IDSubJabatan_Bor);
            $ex->setCellValue('J' . $counter, $row->SubJabatanKry ?? $row->SubJabatanTk);
            $ex->setCellValue('K' . $counter, $row->RKry);
            $ex->setCellValue('L' . $counter, $row->RBor);
            $ex->setCellValue('M' . $counter, $row->TotalKryTk);
            $ex->setCellValue('N' . $counter, $row->CreatedBy);
            $ex->setCellValue('O' . $counter, $row->Periode);
            $counter = $counter + 1;
        endforeach;
        ...
        // header('Pragma: no-cache');
        // header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        // header('Content-Disposition: attachment;filename="Export MPP.xlsx"');
