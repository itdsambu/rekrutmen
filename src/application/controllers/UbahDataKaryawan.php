...
class UbahDataKaryawan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['_getkecamatan'] = $this->m_register->getkecamatan(
            $data['_getData'][0]->ProvinsiID,
            $data['_getData'][0]->KabKotaID
        )->result();
        ...
        $data['_getSubPemborong'] = $this->m_register->getSubPemborong($data['_getData'][0]->CVNama);
        // print_r($data['_getSubPemborong']);
        // die;
    ...
    function updateData()
    {
        ...
        // $test = $this->input->post(null);
        // print_r($test);

        if ($this->session->userdata('dept') == 'HRD') {
            # code...
            $info   = array(
                'CVNama'                    => $cv,
                'Pemborong'                 => $pbr,
                'SubPemborong'              => $subpemborong, // Baru ditambahkan 25/06/2025
                'IDSubPemborong'            => $idsubpemborong, // Baru ditambahkan 25/06/2025
                'Nama'                      => strtoupper($this->input->post('txtNamalengkap')),
                'No_Ktp'                    => strtoupper($this->input->post('txtNoKTP')),
                'No_KK'                     => strtoupper($this->input->post('txtNoKK')),
                ...
                'AccountFacebook'           => $dt_facebook,
                'AccountTwitter'            => $dt_twitter,
                'Account_email'             => $dt_email
            );
        } else {
            $info   = array(
                'CVNama'                    => $cv,
                // 'Pemborong'                 => $_SESSION['Pemborong'],
                // 'Nama'                      => strtoupper($_SESSION['namaTK']),
                // 'No_Ktp'                    => strtoupper($_SESSION['No_Ktp']),
                // 'No_KK'                     => strtoupper($_SESSION['No_KK']),
                'Alamat_KTP'                => strtoupper($this->input->post('txtAlamatKTP')),
                'Alamat'                    => strtoupper($this->input->post('txtAlamat')),
                'RT'                        => $this->input->post('txtRT'),
                'RW'                        => $this->input->post('txtRW'),
                // 'Tempat_Lahir'              => strtoupper($_SESSION['Tempat_Lahir']),
                ...
                // 'NamaBapak'                 => strtoupper($_SESSION['NamaBapak']),
                // 'NamaIbuKandung'            => strtoupper($_SESSION['NamaIbuKandung']),
                'ProfesiOrangTua'           => strtoupper($this->input->post('txtPekerjaanOrtu')),
                'JumlahSaudara'             => $this->input->post('txtJumlahSaudara'),
                'AnakKe'                    => $this->input->post('txtAnakKe'),
