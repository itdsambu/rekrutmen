...
class PotonganBon extends CI_Controller{
   ...
   function send_data(){
    ...
    function simpan_trn_manual_potongan_pemborong(){
        ...
        if($tanggal_periode >= 14 && $tanggal_periode <= 23) {
            $periode = date('Y-m-16',strtotime($tanggal));
        }else{
            if($tanggal_periode == 24 ||$tanggal_periode == 25 || $tanggal_periode == 26 || $tanggal_periode == 27 || $tanggal_periode == 28 || $tanggal_periode == 29 || $tanggal_periode == 30 || $tanggal_periode == 31){
                $bln        = date('m') + 1;
        ...
        // cek user telegram to pemborong
       $user_pbr = $this->M_PotonganBon->GetUserTelPemborong($sub);
       // end user telegram to pemborong
        ...
        if($user_tel == NULL){
            ...
            //  tambah header untuk keterangan 
            ...
            // end tambah header untuk keterangan 
            ...
            $dataTel = array(
                ...
                // 'ToTelegramID'  => 643805139,
                'FirstName'     => $user_tel[0]->FirstName,
                // 'FirstName'     => 'KIKI HENDRIANTO',
                'Messages'      => $pesan,
                'ImageReport'   => NULL,
            ...
            if($user_pbr[0]->FromTelegramID == NULL){
                echo "";
            }else{
                //  Send telegram to pemborong
                $dataTelPbr = array(
                    'UserGroupID'   => 0,
                    'DataFrom'      => 'RO',
                    'ToTelegramID'  => $user_pbr[0]->FromTelegramID,
                    'FirstName'     => $user_pbr[0]->FirstName,
                    'Messages'      => $pesan,
                    'ImageReport'   => NULL,
                    'SendingTime'   => date('Y-m-d H:i:s'),
                    'PendingStatus' => 1,
                ...
                $this->m_monitor->send_info_tel2($dataTelPbr);

                // end send telegram to pemborong
            }
        }
    ...
    function Cicilan()
    {
        ...
        if($tanggal_cicil >= 14 && $tanggal_cicil <= 23) {
            $periode = date('Y-m-16',strtotime($tanggal));
        }else{
            if($tanggal_cicil == 24 ||$tanggal_cicil == 25 || $tanggal_cicil == 26 || $tanggal_cicil == 27 || $tanggal_cicil == 28 || $tanggal_cicil == 29 || $tanggal_cicil == 30 || $tanggal_cicil == 31){
                $bln        = date('m') + 1;
                $periode    = date('Y-0'.$bln.'-1',strtotime($tanggal));
                // echo "OK";
            }else{
    ...
    function simpan_trn_cicilan(){
        ...
        if($nofix == NULL){
            ...
            for ($i = 0; $i < $hitung; $i++) {
                ...
                if($tanggal_ >= 14 && $tanggal_ <= 23) {
                    $tanggalMulai[$i] = date('Y-m-16',strtotime($mulai[$i]));
                }else{
                    if($tanggal_ == 24 || $tanggal_ == 25 || $tanggal_ == 26 || $tanggal_ == 27 || $tanggal_ == 28 || $tanggal_ == 29 || $tanggal_ == 30 || $tanggal_ == 31){
                        $bln        = date('m') + 1;
   ...
   function lihat_pesanan_cicilan(){
    ...
    function simpan_trn_potongan_cicilan(){
       ...
       $cek_durasi  = $this->input->post("txtcek_durasi");
       $mulai      = $this->input->post("txtMulai");
       $periodepotong = $this->input->post('txtperiodepotong');
       $total      = $this->input->post("txtTotal");
       $hitung = count($harga);
...
//                    echo "</pre>";
    ...
    function ExportPDFBySub()
    {
        ...
        if(!empty($cicilan)){
            foreach($cicilan as $key){
                $data['tglMulai'] = $key->TanggalMulai;
                $data['_getDataCicilan'] = $this->M_PotonganBon->getDataCicilan($pbr,$sub,$periode,$key->TanggalMulai);
...
//                    echo "</pre>";
   ...
   // BEGIN :: PRINT SLIP BELANJA
    ...
    function printSlipBelanja(){
        ...
        $data['_getTotalBelanja'] = $this->M_PotonganBon->getDataGrandTotalSembako($hdr[0]->Nofix,$hdrid);
        // // echo "<pre>";
        // // print_r($data['_getTotalBelanja']);
        $ttl_periode_ini = $this->M_PotonganBon->getTotalPeriodeIni($hdr[0]->Nofix,$hdr[0]->PeriodeGajian);
        // // echo "<pre>";
        // // print_r($ttl_periode_ini);
        $data['_totalPeriodeIni']  = $ttl_periode_ini;
        $tglMulai = $this->M_PotonganBon->getTanggalMulai($hdr[0]->Nofix);
        if(empty($tglMulai)){
            $tgl_mulai = '';
            $ttl_cicilan = $this->M_PotonganBon->getPotCicilanPeriodeIni($hdr[0]->Nofix,$tgl_mulai,$hdr[0]->PeriodeGajian);
        ...
        // $data['_totalCicilanPeriodeIni']  = $ttl_cicilan;
        $sisa = $this->M_PotonganBon->GetTK($hdr[0]->Nik,$hdr[0]->Nama,$hdr[0]->Nofix,$hdr[0]->IDSubPemborong);
        $data['_sisa'] = $sisa;
