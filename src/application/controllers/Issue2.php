...
class Issue extends CI_Controller{
    ...
    function boronganIndex(){
        ...
        // $data['cekData'] = $this->m_issue->getCekDataTK();
    ...
    function karyawanIndex(){
        $data['getDept'] = $this->m_issue->getDept();
        $data['getPemb'] = $this->m_issue->getPemborongKaryawan();
        $data['getJbtn'] = $this->m_issue->getJabatan();
        $data['getPend'] = $this->m_issue->getPendidikan();
        $data['getJurs'] = $this->m_issue->getJurusan();
        $data['getSKwn'] = $this->m_issue->getStatusKawin();
        // $data['cekData'] = $this->m_issue->getCekData();
    ...
    // ==== Correct Issue By PSn ====
    function editIssue(){
        $dept   = $this->session->userdata('dept');
        if($dept != 'PSN' && $this->session->userdata('userid') != 'ismo_adm'){
            redirect('welcome/notAkses/2');
        }
    ...
    function viewEditIssue(){
        ...
        $data['getJurs'] = $this->m_issue->getJurusan();
        $data['getSKwn'] = $this->m_issue->getStatusKawin();
        $data['getTran'] = $this->m_issue->setInfoTran($id)->result();
        $row = $this->m_issue->setInfoTran($id)->row();
        $idDept          = $row->DeptID;
        $data['getKrj']  = $this->m_issue->getPekerjaan($idDept);
        $data['getSucc'] = $this->m_issue->countSuccessPermin($id);
    ...
    function doEditIssue(){
        ...
        if($this->input->post('comboPemborong') == 'RSUP'){
