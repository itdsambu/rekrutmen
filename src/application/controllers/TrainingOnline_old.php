...
class TrainingOnlines extends CI_Controller
{
	...
	public function index()
	{
		...
		// $data['_getRM']    = $this->M_TrainingOnline->MasterRuangMeting();

		if ($ip != '192.168.0.194') {
			// print($data['materi']);
			// die();
			$this->template->display('training_online/index', $data);
		} else {
			// moduleElc.php
	...
	// 	$id_dept 		= $this->input->get('iddept');
	// 	$jenis_id 		= $this->input->get('jenis_soal');
	// 	$materi 		= $this->input->get('materi');
	// 	$idHdrSoal 		= $this->input->get('idHdrSoal');
