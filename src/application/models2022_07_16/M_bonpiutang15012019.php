...
class M_Bonpiutang extends CI_Model {
	...
	protected $tblbonawal = '[192.168.3.32].psgborongan.dbo.tblTrnBonAwal';
	...
	public function getallperusahaan(){
		...
		if($id==0){
			...
			if($id==44 || $id==79 || $id==93 || $id==1){
				$sql = "select cperusahaan=CONVERT(varchar(32),HASHBYTES('md5',CONVERT(varchar,idperusahaan)),2), idperusahaan,perusahaan from [192.168.3.32].PSGBorongan.dbo.tblmstperusahaan where perusahaan is not null order by perusahaan";
			}else{
				$sql = "select cperusahaan=CONVERT(varchar(32),HASHBYTES('md5',CONVERT(varchar,idperusahaan)),2), idperusahaan,perusahaan from [192.168.3.32].PSGBorongan.dbo.tblmstperusahaan where perusahaan is not null and idperusahaan=0 order by perusahaan";	
			}
		}else{
		   $sql = "select cperusahaan=CONVERT(varchar(32),HASHBYTES('md5',CONVERT(varchar,idperusahaan)),2), idperusahaan,perusahaan from [192.168.3.32].PSGBorongan.dbo.tblmstperusahaan where perusahaan is not null and idperusahaan=" . $id . " order by perusahaan";	
		}		
	...
	public function getallperusahaanlockunlock($param) //request from datatable
	{
		...
		$query = $this->datatablesql->sql_select($param,
		    ...
		    function($param){
				$tablefrom = '[192.168.3.32].PSGBorongan.dbo.tblMstPerusahaan p ' .
			                 " left join [192.168.3.32].PSGBorongan.dbo.tblTrnBonAwalPTK k on k.idperusahaan = p.IDPerusahaan and k.periode='" . $param['currperiode'] . "' ";
				return $tablefrom;			 
			},
	...
	{
		$query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.BonTKUnLockPT',$param);
		return $query;
	}
	...
	public function get_periodebon(){
		$query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.getperiode',array());
		return $query;
	}
	
	//get islock
	public function islock($param){
		$query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.isLock',$param);
		$row = $query->row();	   
		return $row->isLock;
	}

	public function getminmax($idperusahaan){
		$sql = "select ismin=ISNULL(j.max3month,200000),ismax=isnull(j.maxgeneral,90000000) from [192.168.3.32].PSGBorongan.dbo.tblMstPerusahaan p " .
	...
	public function gettkminmaxbon($param){
		$query = $this->datatablesql->sql_select($param,
			function($param){
				$str = "[192.168.3.32].PSGBorongan.dbo.tblMstPerusahaan p " .
					   "left join tblMstMaxInputBon j on j.idborongan =p.IDPerusahaan";
				return $str;
			},
			function($param){
			...
			,
			function($param){
				$str="IDPerusahaan in (select IDPerusahaan from [192.168.3.32].PSGBorongan.dbo.tblMstPemborong)";
				return $str;
			},
			function($row){
	...
	private function get_tkbonprocess_notlocked($param){
		$query = $this->datatablesql->sql_select($param,
			      ...
			      function($param){
					  ...
					  $str = '[192.168.3.32].PSGBorongan.dbo.tblMstTenagaKerja k ' .
					         'join [192.168.3.32].PSGBorongan.dbo.tblMstPemborong p on p.IDPemborong = k.IDPemborong ' .
							 'join [192.168.3.32].PSGBorongan.dbo.tblMstPerusahaan pe on pe.IDPerusahaan = p.IDPerusahaan ' .
							 'join [192.168.3.32].PSGBorongan.dbo.tblMstDepartemen d on d.IDDept = k.IDDepartemen ' .
							 'join [192.168.3.32].PSGBorongan.dbo.tblMstJabatan j on j.IDJabatan = k.IDJabatan ' .
	...
	public function get_tkbonprocess($param){		
		...
		$periode = $this->db->query('select addonedaylock=convert(varchar(10),DATEADD(DAY,1,lockperiode),126) from [192.168.3.32].PSGBorongan.dbo.vwcurperiode');
		$rowperiode = $periode->row();
	...
	//get data bontk monitor - 
	   ...
	   protected function gettotalbon($param){
	       $sql = "select total=ISNULL(sum(a.BON),0) from [192.168.3.32].PSGBorongan.dbo.tblMstTenagaKerja k  " .
		          "join [192.168.3.32].PSGBorongan.dbo.tblMstPemborong p on p.IDPemborong = k.IDPemborong  " .
				  "join [192.168.3.32].PSGBorongan.dbo.tblMstPerusahaan pe on pe.IDPerusahaan = p.IDPerusahaan " .
				  "join [192.168.3.32].PSGBorongan.dbo.tblMstDepartemen d on d.IDDept = k.IDDepartemen " .
				  "join [192.168.3.32].PSGBorongan.dbo.tblMstJabatan j on j.IDJabatan = k.IDJabatan " .
				  "join [192.168.3.32].PSGBorongan.dbo.tblMstPekerjaan pek on pek.IDPekerjaan = k.IDPekerjaan " .
				  "join [192.168.3.32].PSGBorongan.dbo.tblTrnBonAwal a on a.FixNo =k.FixNo  and a.Periode='" . $param['curperiode'] . "'";
	...
	public function update_bon($param){
	    ...
	    $query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.BonTkUpdate',$params);
		return $query;
	}
	//reset bon tk
	public function reset_bontk($param){
		...
		$query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.BonTkUnlock',$param);
		return $query;
	}
	
	public function bontk_setting_getdata(){
		$query = $this->db->query('select top 1 h=bontklastday,bontkhour=CONVERT(varchar(5),bontkhour) from [192.168.3.32].PSGBorongan.dbo.tblbontk_setting order by id desc');
		return $query;
	}
	
	public function bontk_setting_getdata_save($param){
		$query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.BonTkSetting',$param) ;
		$i = 1;
		return $i;
	}
	...
	public function getperiode_all(){
		$query = "select * from ( " .
				 ...
				 " where Periode not in (select currperiode from [192.168.3.32].PSGBorongan.dbo.vwcurperiode) " .
	...
	public function getbontkcountmonitor($paramdb,$param){
		$query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.BonTKUCountMonitor',$param);
	...
	public function monitorexcellbontk($param){
		...
		$strs = "Select pe.Perusahaan,d.DeptAbbr,j.Jabatan,pek.Pekerjaan,k.Nik,k.Nama," .
					         "Bon=convert(int,isnull(a.BON,0))";							 
		//table
		$strs = $strs .  " from [192.168.3.32].PSGBorongan.dbo.tblMstTenagaKerja k " .
			  "join [192.168.3.32].PSGBorongan.dbo.tblMstPemborong p on p.IDPemborong = k.IDPemborong " .
			  "join [192.168.3.32].PSGBorongan.dbo.tblMstPerusahaan pe on pe.IDPerusahaan = p.IDPerusahaan " .
			  "join [192.168.3.32].PSGBorongan.dbo.tblMstDepartemen d on d.IDDept = k.IDDepartemen " .
			  "join [192.168.3.32].PSGBorongan.dbo.tblMstJabatan j on j.IDJabatan = k.IDJabatan " .
