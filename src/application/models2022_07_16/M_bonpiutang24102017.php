...
class M_Bonpiutang extends CI_Model {
	...
	private function get_tkbonprocess_notlocked($param){
		$query = $this->datatablesql->sql_select($param,
				  ...
				  function($param){
					  ...
					  if($param['islock']==0)
					   $str = "(d.iddept in (16,17,47,48) or pek.idpekerjaan=1) and (k.TanggalKeluar is null and CONVERT(varchar(32),HASHBYTES('md5',CONVERT(varchar,pe.IDPerusahaan)),2)='" . $param['extradata']['perusahaan'] . "')";
					  else
					   $str = '(d.iddept in (16,17,47,48) or pek.idpekerjaan=1) and (k.TanggalKeluar is null and pe.IDPerusahaan=0)';
				      if($stat==0)
	...
	private function get_tkbonprocess_all($param){
		$query = $this->datatablesql->sql_select($param,
				  ...
				  //the field
				  function($param){
					  $str = "d.DeptAbbr,k.FixNo,tglmasuk=convert(varchar(10),k.TanggalMasuk,105),k.Nik,k.Nama,j.Jabatan,j.JabAbbr,pek.Pekerjaan,k.IDPekerjaan,k.IDJabatan,k.IDDepartemen,k.IDPemborong," .
					         "Bon=convert(int,isnull(a.BON,0)) ,islockcv=" . $param['islock'] . " " .
