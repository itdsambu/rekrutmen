...
class M_Bonpiutang extends CI_Model {
	...
	public function getallperusahaan(){
		...
		if($id==0){
			...
			if($id==44 || $id==79 || $id==93 || $id==1){
				$sql = "select cperusahaan=CONVERT(varchar(32),HASHBYTES('md5',CONVERT(varchar,idperusahaan)),2), idperusahaan,perusahaan from tblmstperusahaan where perusahaan is not null order by perusahaan";
			}else{
				$sql = "select cperusahaan=CONVERT(varchar(32),HASHBYTES('md5',CONVERT(varchar,idperusahaan)),2), idperusahaan,perusahaan from tblmstperusahaan where perusahaan is not null and idperusahaan=0 order by perusahaan";	
			}
		}else{
		   $sql = "select cperusahaan=CONVERT(varchar(32),HASHBYTES('md5',CONVERT(varchar,idperusahaan)),2), idperusahaan,perusahaan from tblmstperusahaan where perusahaan is not null and idperusahaan=" . $id . " order by perusahaan";	
		}		
	...
	private function get_tkbonprocess_notlocked($param){
		...
		$this->datatablesql->changedb($this->load->database('PSGBOR', TRUE));
		$query = $this->datatablesql->sql_select($param,
	...
	public function get_tkbonprocessall($param){
		...
		$param['tablebon']= $this->tblbonawal_bor;
