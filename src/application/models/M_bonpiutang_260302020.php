...
class M_Bonpiutang extends CI_Model {
	...
	public function updatebonminmax($param){
		$query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.UpdateMaxMinBonNew',$param);
		return $query;
	}
	...
	public function islock($param){
		$query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.isLockNew',$param);
		$row = $query->row();	   
		return $row->isLock;
	}
	...
	public function getminmax($idperusahaan){
		$sql = "select ismin=ISNULL(j.max3month,200000),ismax=isnull(j.maxgeneral,90000000) from [192.168.3.32].PSGBorongan.dbo.tblMstPerusahaan p " .
		       " left join [192.168.3.32].PSGBorongan.dbo.tblMstMaxInputBon j on j.idborongan =p.IDPerusahaan " .
	...
	public function update_bon($param){
	    ...
	    $query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.BonTkUpdateNew',$params);
		return $query;
	}
	//reset bon tk
	...
	public function getbontkcountmonitor($paramdb,$param){
		$query = $this->exec_sql('[192.168.3.32].PSGBorongan.dbo.BonTKUCountMonitorNew',$param);
