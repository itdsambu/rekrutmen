...
class M_transaksi extends MY_Model
{
    ...
    function listTK($periode, $idpemborong)
    {
        ...
        return $query = $this->db->query("select * from tblTrnKuotaTK where periode=convert(date,'" . $periode . "',105) and CVNama IN (SELECT Perusahaan FROM vwMstPemborong WHERE IDPerusahaan = '" . $idpemborong . "')")->result();
    }
    ...
    {
        return $query = $this->db->query("SELECT distinct a.HeaderID,a.Nama,b.CVNama,a.Periode,b.Status FROM tblTrnKuotaTK AS a INNER JOIN tblMstKoutaPemborong AS b on b.CVNama=a.CVNama WHERE b.CVNama IN (SELECT Perusahaan FROM vwMstPemborong WHERE IDPerusahaan = '" . $idpemborong . "') AND b.Status='UNLOCK' AND a.Periode=convert(date,'" . $periode . "',105) order by Nama ASC")->result();
    }
    ...
    function getTotalKuotaNonPendidikan($periode, $pemborong)
    {
        ...
        $this->db->where("CVNama IN (SELECT Perusahaan FROM vwMstPemborong WHERE IDPerusahaan = '" . $pemborong . "')");
    ...
    {
        $query = $this->db->query("SELECT * FROM tblTrnKuotaTK WHERE CVNama IN (SELECT Perusahaan FROM vwMstPemborong WHERE IDPerusahaan = '" . $pemborong . "') AND convert(varchar,Periode,105)='" . $periode . "'");
        return $query->num_rows();
    }
