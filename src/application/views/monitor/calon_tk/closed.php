<table id="dataTables-closed" class="table table-striped table-bordered table-hover">
    <thead>
        <tr>
            <th>ID</th>
            <th>Nama</th>
            ...
            <th>
                <i class="ace-icon fa fa-user bigger-110 hidden-480"></i> Closed By
            </th>
            <th>Opsi</th>
        </tr>
    </thead>
    ...
    foreach ($_getTenaker as $set):
        ...
        ?>
            ...
            ?></td>
            <td>
                <div class=""><?php echo ucwords(strtolower($set->RegisteredBy));?></div>
                <div class="text-right smaller-80"><?php echo $set->RegisteredDate;?></div>
            </td>
            <td><?php if($set->ClosingBy == NULL){ echo 'Not Record';} else{?>
                <div class=""><?php echo ucwords(strtolower($set->ClosingBy));?></div>
            <div class="text-right smaller-80"><?php echo $set->ClosingDate;?></div><?php }?>
            </td>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#dataTables-closed").on("click", ".detail", function() {
        ...
        $("#dataTables-closed").on("click", ".detailInfo", function() {
            var id = $(this).closest('tr').data('id');
            $.ajax({
