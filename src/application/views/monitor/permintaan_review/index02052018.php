        ...
        <div class="widget-box">
                ...
                <div class="widget-toolbar">
                    <form action="<?php echo site_url('monitor/reviewIssue');?>" method="POST" class="form-inline">
                        <select name="selStatus" id="inputStatus">
                            ...
                            <option value="approved" <?php if($_selStatus == 1){ echo 'selected';}?> >Approved </option>
                            <option value="canceled" <?php if($_selStatus == 2){ echo 'selected';}?> >Canceled </option>
                            <option value="closed" <?php if($_selStatus == 3){ echo 'selected';}?> >Closed </option>
                        </select>
            ...
            <div class="widget-body">
                <div class="widget-main">
                    <?php if ($this->session->userdata('userid') == 'psn_gia'):?>
                        <?php
                    ...
                    <div class="form-group">
                        <div>
                            <div class="radio">
                                <label>
                                    <input name="txtJekel" type="radio" class="ace" value="Harian" onclick="window.location.href ='../monitor/reviewIssue?jenis=harian&status='+$('#inputStatus').val();" <?php if($jenis=='harian'){echo 'checked="checked"';} ?>/>
                                    <span class="lbl"> Harian</span>
                                </label>
                            </div>
                            <div class="radio">
                                <label>
                                    <input name="txtJekel" type="radio" class="ace" value="Karyawan" onclick="window.location.href ='../monitor/reviewIssue?jenis=PSG&status='+$('#inputStatus').val();" <?php if($jenis=='PSG'){echo 'checked="checked"';} ?>/>
                        ...
                        <table id="dataTables-Issue" class="table ">
                            ...
                            ?>
                                ...
                                ?>
                                    ...
                                    <td class="text-center"><?php echo $diidentifikasi;?></td>
                                    <td class="text-center">
                                        <div class="text-left">
                                        <?php if($row->DEPTStatus == 1){ echo $row->DEPTApproval.' <i class="ace-icon fa fa-check green"></i>';}
                                            elseif($row->DEPTStatus == 2){ echo $row->DEPTApproval.' <i class="ace-icon fa fa-times red"></i>';}
                                            else{ echo '<span class="label label-sm label-warning">Pending</span>';}?>
                                    ...
                                    <td class="text-center">
                                        <div class="text-left">
                                        <?php if($row->DIVISIStatus == 1){ echo $row->DIVISIApproval.' <i class="ace-icon fa fa-check green"></i>';}
                                            elseif($row->DIVISIStatus == 2){ echo $row->DIVISIApproval.' <i class="ace-icon fa fa-times red"></i>';}
                                            else{ echo '<span class="label label-sm label-warning">Pending</span>';}?>
                                    ...
                                    <td class="text-center">
                                        <div class="text-left">
                                        <?php if($row->PSNStatus == 1){ echo $row->PSNApproval.' <i class="ace-icon fa fa-check green"></i>';}
                                            elseif($row->PSNStatus == 2){ echo $row->PSNApproval.' <i class="ace-icon fa fa-times red"></i>';}
                                            else{ echo '<span class="label label-sm label-warning">Pending</span>';}?>
                                    ...
                                    <td class="text-center">
                                        <div class="text-left">
                                        <?php if($row->AGMStatus == 1){ echo $row->AGMApproval.' <i class="ace-icon fa fa-check green"></i>';}
                                            elseif($row->AGMStatus == 2){ echo $row->AGMApproval.' <i class="ace-icon fa fa-times red"></i>';}
                                            else { echo '<span class="label label-sm label-warning">Pending</span>';}?>
                                    ...
                                    <td class="text-center">
                                        <div class="text-left">
                                        <?php if($row->VGMStatus == 1){ echo $row->VGMApproval.' <i class="ace-icon fa fa-check green"></i>';}
                                            elseif($row->VGMStatus == 2){ echo $row->VGMApproval.' <i class="ace-icon fa fa-times red"></i>';}
                                            else{ echo '<span class="label label-sm label-warning">Pending</span>';}?>
                                    ...
                                    elseif($row->GeneralStatus == 1){ echo '<span class="label label-sm label-success">Approved</span>';}
                                    ...
                                    <td class="text-center">
                                        ...
                                        <span></span>
                                        <?php
                                            if( $this->session->userdata('userid') == 'psn_gia'):?>
...
<div class="modal fade" id="viewModalcek" tabindex="-1" role="dialog" aria-labelledby="viewModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header"> <!--style="background-color: #008cba">-->
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title">Cek Tenaker Identifikasi by <strong class="green"><?php echo $this->session->userdata('username');?></strong></h4>
            </div>
            <div class="modal-body">
                <input type="hidden" id="inputdetail" name="iddetail">
                <div id="cek" class="well">
                        <!--load tabel dari file detail.php melalui javascript-->
                </div>
            </div>
            <div class="modal-footer">
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#dataTables-Issue").on("click", ".cek", function() {
            var id = $(this).closest('tr').data('id');
            $.ajax({
                url:"<?php echo site_url('monitor/viewcekDetail');?>",
                type:"POST",
                data:"kode="+id,
                datatype:"json",
                cache:false,
