...
$header5 = array(
 ...
 $Bold= array(
        ...
        //Marge Cells
         ...
         $objPHPExcel->getActiveSheet()->getStyle('A2:K2')->applyFromArray($header4);
         ...
         $objPHPExcel->getActiveSheet()->getStyle('J5:J5')->applyFromArray($style_col);
         $objPHPExcel->getActiveSheet()->getStyle('K5:K5')->applyFromArray($style_col);        

         $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A2','REKAP POTONGAN BON TENAGA KERJA') 
                ->setCellValue('A5','NO')
                ->setCellValue('B5','Nofix')
                ->setCellValue('C5','Nik')
                ->setCellValue('D5','Nama Lengkap')
                ->setCellValue('E5','Dept')
                ->setCellValue('F5','Sub Pemborong')
                ->setCellValue('G5','Potongan Sembako')
                ->setCellValue('H5','Potongan Cicilan')
                ->setCellValue('I5','Total')
                ->setCellValue('J5','Dipotong Saat Kalkulasi')
                ->setCellValue('K5','Sisa Bon')
        ...
        foreach($getDataMonitor as $get=> $key){
            ...
            $objPHPExcel->getActiveSheet()->getStyle('D'.$row.':D'.$row)->applyFromArray($header3);
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('B'.$row,$key->Nofix)
                        ->setCellValue('C'.$row,$key->Nik)
                        ->setCellValue('D'.$row,$key->Nama)
                        ->setCellValue('E'.$row,$key->Bagian)
                        ->setCellValue('F'.$row,$key->NamaSub)
                        ->setCellValue('G'.$row,number_format($key->Pot_Sembako,0,".",","))
                        ->setCellValue('H'.$row,number_format($key->Cicilan,0,".",","))
                        ->setCellValue('I'.$row,number_format(($key->Cicilan + $key->Pot_Sembako),0,".",","))
                        
            ;
            $total_sembako_all += $key->Pot_Sembako;
            $total_cicilan_all += $key->Cicilan;

            $row++;
        }
        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('F'.$row,'TOTAL')
                ->setCellValue('G'.$row,'Rp.'.number_format($total_sembako_all,0,".",","))
                ->setCellValue('H'.$row,'Rp.'.number_format($total_cicilan_all,0,".",","))
                ->setCellValue('I'.$row,'Rp.'.number_format(($total_cicilan_all + $total_sembako_all),0,".",","))
        ;
        ...
        header('Content-Disposition: attachment;filename="List Bon Tenaga Kerja Per Sub.xlsx"');
