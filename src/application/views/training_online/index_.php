...
<script type="text/javascript">
	...
	function start(){
		...
		if(jam_sekarang >= jam && jam_sekarang <= jam_akhir){
            $.ajax({
				type: "POST",
				dataType: "html",
				url : "<?php echo site_url('TrainingOnline/getDataSoal/')?>"+materi+"/"+jenis_soal+"/"+idHdrSoal,
		...
		for(var i= 0; i<=jmlBaris;i++){
			...
			for(var j = 0; j < jawaban.length; j++){
				if(jawaban[j].checked){
		        	data_jawaban.push({"soal_id": soal_id.value, "jawaban":jawaban[j].value});
		      	}
