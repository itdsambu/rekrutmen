				...
				<div class="card m-b-20">
					<div class="card-body">
						...
						<div class="form-group row">
							...
							<!-- <div class="col-sm-4">
						        	<select class="form-control" name="txtRuangan" id="ruangan">
						        		<option value="">- Pilih -</option>
						        		<?php foreach ($_getRM as $RM) {
											echo "<option value='$RM->IDrm'>" . $RM->RuangMeeting . "</option>";
										} ?>
						        	</select>
						        </div> -->
...
<script type="text/javascript">
	...
	function get_list_tkb() {
		var nik = $('#nik_id').val();
		var dept = $('#dept').val();
		var dept_text = $('#dept').find('option:selected').text();
		console.log(dept);
		//alert(nik);
		$.ajax({
			type: "POST",
			dataType: "html",
			url: "<?php echo site_url('TrainingOnline/cari_tenaga_kerja/') ?>" + nik + "/" + dept,
			success: function(msg) {
                if (msg != ''){
                    $('#ajaxfrommodal').html(msg);
                } else {
                    $('#ajaxfrommodal').html(`<div class="form-group row">
                                                <label for="example-search-input" class="col-sm-2 col-form-label">NIK</label>
                                                <div class="col-sm-4">
                                                    <input type="text" name="txtNik" id="nik_id" class="form-control" onchange="get_list_tkb();" value="" placeholder="Input NIK" required>
                                                        <input type="hidden" name="txtfixReg" id="fixreg" class="form-control" value="" readonly required>
                                                        <input type="hidden" name="txtPerson" id="idPerson" class="form-control" value="" readonly required>
                                                        <input type="hidden" name="txtStatusPerson" id="karyawanSt" class="form-control" value="" readonly required>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="example-search-input" class="col-sm-2 col-form-label">Nama</label>
                                                <div class="col-sm-4">
                                                    <input type="text" name="txtNama" id="nama_lengkap" class="form-control" value="" readonly required placeholder="Nama Auto">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="example-search-input" class="col-sm-2 col-form-label">Bagian</label>
                                                <div class="col-sm-4">
                                                    <input type="text" name="txtBagian" id="bagian" class="form-control" value="" readonly required placeholder="Bagian Auto">
                                                    <input type="hidden" name="txtBagianID" id="bagianID" class="form-control" value="" readonly required placeholder="Bagian Auto">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="example-search-input" class="col-sm-2 col-form-label">Jabatan</label>
                                                <div class="col-sm-4">
                                                    <input type="text" name="txtJabatan" id="jabatan" class="form-control" value="" readonly required placeholder="Jabatan Auto">
                                                </div>
                                            </div>`);
	...
	function start() {
		...
		var ruangan = $('#ruangan').val();
		var karyawanSt = $("#karyawanSt").val();
		var fixReg = $("#fixreg").val();
